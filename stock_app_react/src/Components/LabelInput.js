...
const LabelInput = forwardRef(({ svg = null, state, setState, type, formName = null, form = null, registerHandler = null, children = null, label }, ref) => {
  const activeStatus = state ? "translate-y-[-14px]" : "translate-y-0"
  const clickOutside = (state, ref, setState) => {
    return e => {
      if (state && ref.current && ref.current.value.length === 0) {
        setState(false)
      }
    }
  }
  ...
  return (
    <div className='bg-[#ECF0F1] w-full h-[50px] flex justify-start items-center gap-[5px] font-bold text-primary-gray border border-custom-gray'>
      ...
      <div className="flex flex-col align-center justify-center h-full text-[13px] overflow-hidden w-[90%]">
        ...
        <input ref={ref} onKeyUp={() => setState(true)} onClick={() => setState(true)} className='text-black bg-transparent w-full relative outline-none' type={type} />
      </div>
      {children}
    </div>
  )
